...
function renderBills(){
  ...
  document.querySelectorAll(".pagar").forEach(btn=>btn.addEventListener("click",()=>openPixModal(Number(btn.dataset.id))));
}
...
function crc16(str){let crc=0xFFFF;const poly=0x1021;const data=new TextEncoder().encode(str);for(let b of data){crc^=(b<<8);for(let i=0;i<8;i++){crc=(crc&0x8000)?((crc<<1)^poly)&0xFFFF:(crc<<1)&0xFFFF;}}return crc.toString(16).padStart(4,"0");}
